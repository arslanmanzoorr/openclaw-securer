...
// ClawHub supply-chain attack mitigation: block skills that install the compromised registry CLI.
const BLOCKED_SKILL_NAMES = new Set(["clawhub", "claw-hub", "clawhub-cli"]);

export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  if (BLOCKED_SKILL_NAMES.has(params.skillName.toLowerCase())) {
    return {
      ok: false,
      message:
        "Installation of ClawHub-related skills is blocked due to supply-chain security concerns. " +
